...
З початкових умов $$b_k = - f_k / \mu_k^2$$, $$a_k = 0$$, тоді

\begin{equation}
	T_k(t) = - \frac{f_k}{\mu_k^2} \cos(\mu_k t) + \frac{f_k}{\mu_k^2}.
\end{equation}

Тоді будемо шукати загальний розв'язок у вигляді

\begin{equation}
	u(r, t) = \Sum_{k = 0}^{\infty} R_k(r) T_k(t) = \Sum_{k = 0}^{\infty} \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \cdot \left(- \frac{f_k}{\mu_k^2} \cos(\mu_k t) + \frac{f_k}{\mu_k^2}\right).
\end{equation}

Підставляємо його у початкову умову
