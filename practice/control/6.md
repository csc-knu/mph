...
\begin{equation}
	u(r, t) = \Sum_{k = 0}^{\infty} R_k(r) T_k(t) = \Sum_{k = 0}^{\infty} c_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \cdot \left(\left( f_k - \frac{f_k}{\mu_k^2} \right) \cos(\mu_k t) + \frac{f_k}{\mu_k^2}\right).
\end{equation}

Підставляємо його у початкову умову $$u_(r, 0) = u_0(r)$$:

\begin{equation}
	u_0(r) = u(r, 0) = \Sum_{k = 0}^{\infty} c_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \cdot \left(\left( f_k - \frac{f_k}{\mu_k^2} \right) \cos(\mu_k \cdot 0) + \frac{f_k}{\mu_k^2}\right).
\end{equation}

Спрощуємо до

\begin{equation}
	u_0(r) = \Sum_{k = 0}^{\infty} c_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \cdot \left(f_k - \frac{f_k}{\mu_k^2} + \frac{f_k}{\mu_k^2}\right),
\end{equation}

і далі до

\begin{equation}
	u_0(r) = \Sum_{k = 0}^{\infty} c_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \cdot f_k.
\end{equation}
