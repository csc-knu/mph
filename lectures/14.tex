...
\begin{align}
	...
	u_y &= u_\xi \cdot \xi_y + u_\eta \cdot \eta_y, \\
...
\begin{enumerate}
	...
	Тоді отримаємо канонічну форму запису рівняння еліптичного вигляду:
	\begin{equation}
...
\begin{solution}
	...
	обчислимо похідні 
	\begin{align*}
		u_x &= u_\alpha 2 x + u_\beta \cdot 0, \\
		u_y &= u_\alpha \cdot 0 + u_\beta 2 y, \\
		u_{xx} &= u_{\alpha\alpha} 4x^2 + 2 u_{\alpha\beta} \cdot 0 + u_{\beta\beta} \cdot 0 + u_\alpha \cdot 2 + u_\beta \cdot 0, \\
		u_{yy} &= u_{\alpha\alpha} \cdot 0 + 2 u_{\alpha\beta} \cdot 0 + u_{\beta\beta} 4y^2 + u_\alpha \cdot 0 + u_\beta \cdot 2.
	\end{align*}
...
\begin{solution}
	...
	Для диференціального рівняння маємо нові змінні: $\vec \xi = B^T \vec x$. Або в координатній формі:
	\begin{system*}
		\xi &= x, \\
		\eta &= y - x, \\
		\zeta &= x - y / 2 + z / 2.
...
\begin{example}
	...
	\begin{equation}
		\left( \Delta \vec U + \frac{m}{m - 2} \nabla \Big(\nabla \cdot \vec U\Big)\right) = - \frac{1}{G} \vec X.
	\end{equation}
\end{example}
...
\begin{solution}
	...
	\begin{equation}
		A_0(D) = \begin{pmatrix}
			\Delta + \lambda \frac{\partial^2}{\partial x^2} & \lambda \frac{\partial^2}{\partial x\partial y} & \lambda \frac{\partial^2}{\partial x\partial z} \\
			\lambda \frac{\partial^2}{\partial y\partial x} & \Delta + \lambda \frac{\partial^2}{\partial y^2} & \lambda \frac{\partial^2}{\partial y\partial z} \\
			\lambda \frac{\partial^2}{\partial z\partial x} & \Delta + \lambda \frac{\partial^2}{\partial z \partial y} & \lambda \frac{\partial^2}{\partial z^2}
		\end{pmatrix}
	\end{equation}
...
Відповідна характеристична форма матиме вигляд:
 				(7.25),    де  ,   - квадрат швидкості звуку. Розкриваючи визначник (7.25) отримаємо співвідношення  	 (7.26)
Розглядаючи (7.26) як поліном відносно змінної  , яка відповідає змінній часу   у системі рівнянь, отримаємо для довільних дійсних значень вектора    чотири дійсних кореня, а саме  .
Таким чином система рівнянь гідродинаміки (7.24) має гіперболічний тип.
...
Характеристична форма  запишеться у вигляді:
 , де  . Враховуючи, що характеристична форма містить лише три змінних, а рівняння містить чотири, можемо зробити висновок, що система (7.27) є параболічною.
\end{solution}
