...
\begin{equation}
	u(r, t) = \Sum_{k = 0}^{\infty} R_k(r) T_k(t) = \Sum_{k = 0}^{\infty} c_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \left(\left( f_k - \frac{f_k}{\mu_k^2} \right) \cos(\mu_k t) + \frac{f_k}{\mu_k^2}\right).
\end{equation}

Підставляємо його у початкову умову $$u_(r, 0) = u_0(r)$$:
...
\begin{equation}
	u_0(r) = \Sum_{k = 0}^{\infty} c_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) f_k.
\end{equation}

Звідси знаходимо $$c_k$$ розкладаючи $$u_0(r)$$ по систему власних функцій ($$R_k$$) у ряд:

\begin{equation}
	u_0(r) = \Sum_{k = 0}^{\infty} u_k \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right),
\end{equation}

тоді $$c_k = u_k / f_k$$, і остаточно розв'язок:

\begin{equation}
	u(r, t) = \Sum_{k = 0}^{\infty} \frac{u_k}{f_k} \left( \frac{J_0(\mu_k r)}{J_0(\mu_k r_1)} - \frac{N_0(\mu_k r)}{N_0(\mu_k r_1)} \right) \left(\left( f_k - \frac{f_k}{\mu_k^2} \right) \cos(\mu_k t) + \frac{f_k}{\mu_k^2}\right).
\end{equation}
