...
Для побудови функції Гріна граничних задач \eqref{eq:4.20}, \eqref{eq:4.21} на півпрямій використаємо метод відображення теплових джерел.
...
\begin{equation}
    E_1(x, \xi, t - \tau) = \frac{\theta(t - \tau)}{2 a \sqrt{t - \tau}} \exp\left\{-\frac{|x - \xi|^2}{2 a^2 (t - \tau)}\right\} - \frac{\theta(t-\tau)}{2a\sqrt{t-\tau}} \exp\left\{-\frac{|x + \xi|^2}{2a^2 (t-\tau)}\right\}.
\end{equation}
...
\begin{equation}
    E_2(x, \xi, t- \tau) = \frac{\theta(t-\tau)}{2a\sqrt{t-\tau}} \exp\left\{-\frac{|x - \xi|^2}{2a^2 (t-\tau)}\right\} + \frac{\theta(t-\tau)}{2a\sqrt{t-\tau}} \exp\left\{\frac{|x + \xi|^2}{2a^2 (t-\tau)}\right\}.
\end{equation}
